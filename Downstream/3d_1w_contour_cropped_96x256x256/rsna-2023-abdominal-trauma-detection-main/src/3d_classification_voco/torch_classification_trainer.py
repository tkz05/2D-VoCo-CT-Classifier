...
def train(
        ...
        optimizer, device,
        scheduler=None, amp=False , mode = 'rest_organs'
):
    """
    mode:
        "all" -> 訓練所有器官 (extravasation + bowel + kidney + liver + spleen)
        "extravasation" -> 只訓練 extravasation
        "rest_organs" -> 只訓練 bowel, kidney, liver, spleen
    """
    """
    ...
    """
    print(f"Now mode is : {mode}")
    model.train()
    progress_bar = tqdm(training_loader, dynamic_ncols=True)
    ...
    for step, (inputs, bowel_targets, extravasation_targets, kidney_targets, liver_targets, spleen_targets, any_targets) in enumerate(progress_bar):
        ...
        #----------------------------------------------------#
        loss_total = 0.0
        # print(f"bowel_outputs : {bowel_outputs}, bowel_targets : {bowel_targets.view(-1, 1)}")
        # print(f"kidney_outputs : {kidney_outputs}, kidney_targets : {kidney_targets}")
        # print(f"liver_outputs : {liver_outputs}, liver_targets : {liver_targets}")
        # print(f"spleen_outputs : {spleen_outputs}, spleen_targets : {spleen_targets}")

        if mode in ["all", "extravasation"]:
            extravasation_loss = extravasation_criterion(extravasation_outputs, extravasation_targets.view(-1, 1), extravasation_weights)
            loss_total += extravasation_loss
            running_loss_extravasation += extravasation_loss.item() * len(inputs)

        if mode in ["all", "rest_organs"]:
            ...
            loss_total += bowel_loss + kidney_loss + liver_loss + spleen_loss
        ...
        if scheduler is not None:
            scheduler.step()
        # print(f"loss.item() : {loss_total.item()}")
        # if hasattr(model, "recorded_grads"):
        #     print("🧠 Gradient norms for backbone:")
        #     for name, grad_norm in model.recorded_grads.items():
        ...
        progress_bar.set_description(f'lr: {lr:.8f} - '
                             f'total: {running_loss_total / len(training_loader.sampler):.4f} '
                             f'bowel: {running_loss_bowel / len(training_loader.sampler):.4f} '
                             f'extravasation: {running_loss_extravasation / len(training_loader.sampler):.4f} '
                             f'kidney: {running_loss_kidney / len(training_loader.sampler):.4f} '
                             f'liver: {running_loss_liver / len(training_loader.sampler):.4f} '
                             f'spleen: {running_loss_spleen / len(training_loader.sampler):.4f}')

    training_loss = {'loss_total': running_loss_total / len(training_loader.sampler)}
    if mode in ["all", "rest_organs"]:
        training_loss.update({
            'loss_bowel': running_loss_bowel / len(training_loader.sampler),
            'loss_kidney': running_loss_kidney / len(training_loader.sampler),
            'loss_liver': running_loss_liver / len(training_loader.sampler),
            'loss_spleen': running_loss_spleen / len(training_loader.sampler)
        })
    if mode in ["all", "extravasation"]:
        training_loss['loss_extravasation'] = running_loss_extravasation / len(training_loader.sampler)
        #----------------------------------------------------#
...
def validate(
        ...
        bowel_criterion, extravasation_criterion, kidney_criterion, liver_criterion, spleen_criterion,
        device, amp=False , mode='rest_organs'
):
    ...
    model.eval()
    progress_bar = tqdm(validation_loader, dynamic_ncols=True)
    ...
    for step, (inputs, bowel_targets, extravasation_targets, kidney_targets, liver_targets, spleen_targets, any_targets) in enumerate(progress_bar):
        ...
        progress_bar.set_description(f'validation total: {running_loss_total / len(validation_loader.sampler):.4f} '
                                    f'bowel: {running_loss_bowel / len(validation_loader.sampler):.4f} '
                                    f'extravasation: {running_loss_extravasation / len(validation_loader.sampler):.4f} '
                                    f'kidney: {running_loss_kidney / len(validation_loader.sampler):.4f} '
                                    f'liver: {running_loss_liver / len(validation_loader.sampler):.4f} '
                                    f'spleen: {running_loss_spleen / len(validation_loader.sampler):.4f}')
    ...
    validation_loss = {'loss_total': running_loss_total / len(validation_loader.sampler)}
    if mode in ["all", "rest_organs"]:
        validation_loss.update({
            'loss_bowel': running_loss_bowel / len(validation_loader.sampler),
            'loss_kidney': running_loss_kidney / len(validation_loader.sampler),
            'loss_liver': running_loss_liver / len(validation_loader.sampler),
            'loss_spleen': running_loss_spleen / len(validation_loader.sampler)
        })
    if mode in ["all", "extravasation"]:
        validation_loss['loss_extravasation'] = running_loss_extravasation / len(validation_loader.sampler)
...
if __name__ == '__main__':
    ...
    df = pd.read_parquet(settings.DATA / 'datasets' / dataset / 'metadata_80folds.parquet')#
    # df = pd.read_parquet(settings.DATA / 'datasets' / dataset / 'metadata.parquet')
    # df = pd.read_parquet(settings.DATA / 'downstream_metadata.parquet')#
    
    # df_train = pd.read_csv(settings.DATA / 'rsna-2023-abdominal-trauma-detection' / '80_train_with_folds.csv')
    df_train = pd.read_csv(settings.DATA / 'rsna-2023-abdominal-trauma-detection' / 'train.csv')
    # df_train = pd.read_csv(settings.DATA / 'pretrain_val_labeled.csv')
    
    df_folds = pd.read_csv(settings.DATA / '80_folds.csv' )#
    # df_folds = pd.read_csv(settings.DATA / 'folds.csv')
    # df_folds = pd.read_csv(settings.DATA / 'downstream_folds.csv')#
    ...
    if args.mode == 'training':
        ...
        for fold in config['training']['folds']:
            ...
            print(f"batch size = {config['training']['training_batch_size']}")
            ...
            for step, (inputs, bowel_targets, extravasation_targets, kidney_targets, liver_targets, spleen_targets, any_targets) in enumerate(training_loader):
                print(f"[STEP {step}] batch size: {len(inputs)}")
                print(f"  Bowel targets: {bowel_targets.view(-1).tolist()}")
                print(f"  Patient IDs or info: {any_targets}")
                if step >= 3:
                    break  
            ...
            print(f"amp = {amp}")
            print(f"model name = {config['model']['model_args']['model_name']}")
            best_epoch = 1
